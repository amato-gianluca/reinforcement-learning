...
def policy_evaluation(mdp: MDP[S,A], pi: Policy[S,A], gamma: float, theta: float,
    ...
    theta is used to detect convergenze of successive approximations. The smaller is theta, the more precise
    is the result. If v is given, computation starts from the provided state-value function, otherwise
    computation starts from an always null state-value function."""
    if v is None:
...
def policy_iteration(mdp: MDP[S,A], gamma: float, theta: float, pi: Optional[Policy[S,A]] = None) -> Tuple[Policy[S,A], StateValue[S]]:
    """Return best policy and state-value function for an MDP. gamma and theta have the same meanong as in
    policy_evaluation, and pi is an optional initial policy"""
    if pi is None:
